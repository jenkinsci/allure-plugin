...
import static ru.yandex.qatools.allure.jenkins.utils.ToolUtils.findJavaExecutable;
...
public class AllureReportPublisher extends Recorder implements Serializable, MatrixAggregatable {
    ...
    private boolean generateReport(FilePath results, AbstractBuild<?, ?> build, Launcher launcher,
        ...
        AllureCommandlineInstallation commandline = getDescriptor().
                getCommandlineInstallation(getConfig().getCommandlineName());
        ...
        Map<String, String> buildEnv = build.getEnvironment(listener);
        File javaExecutable = findJavaExecutable(getConfig().getJdk(), buildEnv, build.getProject(), listener);
        ...
        arguments.add("-cp").add(commandline.getClassPath(launcher));
        arguments.addKeyValuePair("-D", "allure.home", commandline.getHome(), false);
        arguments.addKeyValuePair("-D", "allure.config", configDirectory.getRemote(), false);
        arguments.add("ru.yandex.qatools.allure.CommandLine").add("generate");
        arguments.addQuoted(results.getRemote());
        arguments.add("-o").addQuoted(reportDirectory.getRemote());
        launcher.launch().cmds(arguments).stdout(listener).pwd(build.getWorkspace()).join();
